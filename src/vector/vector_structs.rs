...
unsafe impl<'a, T: 'a, const D: usize> Get for ReferringOwnedArray<'a, T, D> {
    ...
    #[inline]
    unsafe fn get_inputs(&mut self, index: usize) -> Self::Inputs {
        unsafe { &*(self.0.get_unchecked(index) as *const T) }
    }
    #[inline]
    fn process(&mut self, _: usize, inputs: Self::Inputs) -> (Self::Item, Self::BoundItems) {
        (inputs, ())
...
unsafe impl<'a, T, const D: usize> Get for &'a mut [T; D] {
    ...
    #[inline]
    unsafe fn get_inputs(&mut self, index: usize) -> Self::Inputs {
        unsafe { &mut *(self.get_unchecked_mut(index) as *mut T) }
    }
    #[inline]
    fn process(&mut self, _: usize, inputs: Self::Inputs) -> (Self::Item, Self::BoundItems) {
        (inputs, ())
...
unsafe impl<V: VectorLike + ?Sized> Get for Box<V> {
    ...
    #[inline]
    unsafe fn get_inputs(&mut self, index: usize) -> Self::Inputs {
        unsafe { (debox(self)).get_inputs(index) }
    }
    #[inline]
    unsafe fn drop_inputs(&mut self, index: usize) {
        unsafe { (debox(self)).drop_inputs(index) }
    }
    #[inline]
    fn process(&mut self, index: usize, inputs: Self::Inputs) -> (Self::Item, Self::BoundItems) {
        (debox(self)).process(index, inputs)
    }
}
...
impl<V: VectorLike + ?Sized> HasReuseBuf for Box<V> {
    ...
    unsafe fn drop_1st_buf_index(&mut self, index: usize) {
        unsafe { (debox(self)).drop_1st_buf_index(index) }
    }
    #[inline]
    unsafe fn drop_2nd_buf_index(&mut self, index: usize) {
        unsafe { (debox(self)).drop_2nd_buf_index(index) }
    }
    #[inline]
