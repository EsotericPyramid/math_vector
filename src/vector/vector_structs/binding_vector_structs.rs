...
pub struct VecBind<V: VectorLike<FstHandleBool = Y>> {
    pub(crate) vec: V,
}

unsafe impl<V: VectorLike<FstHandleBool = Y>> Get for VecBind<V>
where
...
impl<V: VectorLike<FstHandleBool = Y>> HasOutput for VecBind<V>
where
    (V::OutputBool, V::FstOwnedBufferBool): FilterPair,
{
    type OutputBool = <(V::OutputBool, V::FstOwnedBufferBool) as TyBoolPair>::Or;
    type Output = <(V::OutputBool, V::FstOwnedBufferBool) as FilterPair>::Filtered<
        V::Output,
        V::FstOwnedBuffer,
    >;
    ...
    unsafe fn output(&mut self) -> Self::Output {
        unsafe {
            <(V::OutputBool, V::FstOwnedBufferBool) as FilterPair>::filter(
                self.vec.output(),
...
impl<V: VectorLike<FstHandleBool = Y>> HasReuseBuf for VecBind<V>
where
    (V::BoundHandlesBool, Y): FilterPair,
{
    ...
    unsafe fn assign_bound_bufs(&mut self, index: usize, val: Self::BoundTypes) {
        unsafe {
            let (bounded_vals, new_bound_val) =
                <(V::BoundHandlesBool, Y) as FilterPair>::defilter(val);
...
/// struct binding the vector's item to the buffer in the first slot which is linked to output without cleanly filtering the output
pub struct VecRawBind<V: VectorLike<FstHandleBool = Y>> {
    pub(crate) vec: V,
}

unsafe impl<V: VectorLike<FstHandleBool = Y>> Get for VecRawBind<V>
where
    (V::BoundHandlesBool, Y): FilterPair,
{
    type GetBool = N;
...
impl<V: VectorLike<FstHandleBool = Y>> HasOutput for VecRawBind<V>
where
    (V::OutputBool, V::FstOwnedBufferBool): TyBoolPair,
{
    type OutputBool = <(V::OutputBool, V::FstOwnedBufferBool) as TyBoolPair>::Or;
    type Output = (V::Output, V::FstOwnedBuffer);

    #[inline]
    unsafe fn output(&mut self) -> Self::Output {
        unsafe { (self.vec.output(), self.vec.get_1st_buffer()) }
    }
...
impl<V: VectorLike<FstHandleBool = Y>> HasReuseBuf for VecRawBind<V>
where
    (V::BoundHandlesBool, Y): FilterPair,
{
    type FstHandleBool = N;
...
pub struct VecMapBind<V: VectorLike<FstHandleBool = Y>, F: FnMut(V::Item) -> (I, B), I, B> {
    pub(crate) vec: V,
    pub(crate) f: F,
}

unsafe impl<V: VectorLike<FstHandleBool = Y>, F: FnMut(V::Item) -> (I, B), I, B> Get
    for VecMapBind<V, F, I, B>
where
    (V::BoundHandlesBool, Y): FilterPair,
{
    type GetBool = Y;
    ...
    fn process(&mut self, index: usize, inputs: Self::Inputs) -> (Self::Item, Self::BoundItems) {
        ...
        (
            processed_item,
            <(V::BoundHandlesBool, Y) as FilterPair>::filter(bound, processed_bound),
        )
    }
}

impl<V: VectorLike<FstHandleBool = Y>, F: FnMut(V::Item) -> (I, B), I, B> HasOutput
    for VecMapBind<V, F, I, B>
where
    (V::OutputBool, V::FstOwnedBufferBool): FilterPair,
{
...
impl<V: VectorLike<FstHandleBool = Y>, F: FnMut(V::Item) -> (I, B), I, B> HasReuseBuf
    for VecMapBind<V, F, I, B>
where
    (V::BoundHandlesBool, Y): FilterPair,
{
...
pub struct VecHalfBind<V: VectorLike<FstHandleBool = Y>> {
    pub(crate) vec: V,
}
...
unsafe impl<V: VectorLike<FstHandleBool = Y>> Get for VecHalfBind<V>
where
    (V::BoundHandlesBool, Y): FilterPair,
{
    type GetBool = N;
...
impl<V: VectorLike<FstHandleBool = Y>> HasOutput for VecHalfBind<V>
where
    (V::OutputBool, V::FstOwnedBufferBool): TyBoolPair,
{
    type OutputBool = <(V::OutputBool, V::FstOwnedBufferBool) as TyBoolPair>::Or;
...
impl<V: VectorLike<FstHandleBool = Y>> HasReuseBuf for VecHalfBind<V>
where
    (V::BoundHandlesBool, Y): FilterPair,
{
    type FstHandleBool = N;
