...
pub struct VecRawBind<V: VectorLike<FstHandleBool = Y>> {pub(crate) vec: V} 

unsafe impl<V: VectorLike<FstHandleBool = Y>> Get for VecRawBind<V> where (V::BoundHandlesBool, Y): FilterPair {
    type GetBool = N;
    type Inputs = V::Inputs;
    type Item = ();
    type BoundItems = <(V::BoundHandlesBool, Y) as FilterPair>::Filtered<V::BoundItems, V::Item>;
...
impl<V: VectorLike<FstHandleBool = Y>> HasOutput for VecRawBind<V> where (V::OutputBool, V::FstOwnedBufferBool): TyBoolPair {
    type OutputBool = <(V::OutputBool, V::FstOwnedBufferBool) as TyBoolPair>::Or;
    type Output = (V::Output, V::FstOwnedBuffer);
    
    #[inline]
    unsafe fn output(&mut self) -> Self::Output { unsafe {
        (self.vec.output(), self.vec.get_1st_buffer())
    }}
...
impl<V: VectorLike<FstHandleBool = Y>> HasReuseBuf for VecRawBind<V> where (V::BoundHandlesBool, Y): FilterPair {
    type FstHandleBool = N;
    type SndHandleBool = V::SndHandleBool;
    type BoundHandlesBool = <(V::BoundHandlesBool, Y) as TyBoolPair>::Or;
    type FstOwnedBufferBool = N;
