use crate::{util_traits::*, vector::vec_util_traits::*};

/// Offsets (with rolling over) each element up by offset
pub struct VecOffset<V: VectorLike> {
    pub(crate) vec: V,
...
impl<V: VectorLike> VecOffset<V> {
    ...
    fn offset_index(&self, index: usize) -> usize {
        ...
        if offset_index >= index {
            // FIXME: see if there is a better way to detect overflows
...
unsafe impl<V: VectorLike> Get for VecOffset<V> {
    ...
    unsafe fn get_inputs(&mut self, index: usize) -> Self::Inputs {
        unsafe { self.vec.get_inputs(self.offset_index(index)) }
    }
    #[inline]
    unsafe fn drop_inputs(&mut self, index: usize) {
        unsafe { self.vec.drop_inputs(self.offset_index(index)) }
    }
    #[inline]
    fn process(&mut self, index: usize, inputs: Self::Inputs) -> (Self::Item, Self::BoundItems) {
        self.vec.process(self.offset_index(index), inputs)
    }
}
...
impl<V: VectorLike> HasReuseBuf for VecOffset<V> {
    ...
    unsafe fn assign_1st_buf(&mut self, index: usize, val: Self::FstType) {
        unsafe { self.vec.assign_1st_buf(self.offset_index(index), val) }
    }
    #[inline]
    unsafe fn assign_2nd_buf(&mut self, index: usize, val: Self::SndType) {
        unsafe { self.vec.assign_2nd_buf(self.offset_index(index), val) }
    }
    #[inline]
    unsafe fn assign_bound_bufs(&mut self, index: usize, val: Self::BoundTypes) {
        unsafe { self.vec.assign_bound_bufs(self.offset_index(index), val) }
    }
    #[inline]
    ...
    unsafe fn drop_1st_buf_index(&mut self, index: usize) {
        unsafe { self.vec.drop_1st_buf_index(self.offset_index(index)) }
    }
    #[inline]
    unsafe fn drop_2nd_buf_index(&mut self, index: usize) {
        unsafe { self.vec.drop_2nd_buf_index(self.offset_index(index)) }
    }
    #[inline]
    unsafe fn drop_bound_bufs_index(&mut self, index: usize) {
        unsafe { self.vec.drop_bound_bufs_index(self.offset_index(index)) }
    }
}

/// Reverses the elements in the vector
pub struct VecReverse<V: VectorLike> {
    pub(crate) vec: V,
...
unsafe impl<V: VectorLike> Get for VecReverse<V> {
    ...
    #[inline]
    unsafe fn get_inputs(&mut self, index: usize) -> Self::Inputs {
        unsafe { self.vec.get_inputs(self.reverse_index(index)) }
    }
    #[inline]
    unsafe fn drop_inputs(&mut self, index: usize) {
        unsafe { self.vec.drop_inputs(self.reverse_index(index)) }
    }
    #[inline]
    fn process(&mut self, index: usize, inputs: Self::Inputs) -> (Self::Item, Self::BoundItems) {
        self.vec.process(self.reverse_index(index), inputs)
    }
}
...
impl<V: VectorLike> HasReuseBuf for VecReverse<V> {
    ...
    #[inline]
    unsafe fn assign_1st_buf(&mut self, index: usize, val: Self::FstType) {
        unsafe { self.vec.assign_1st_buf(self.reverse_index(index), val) }
    }
    #[inline]
    unsafe fn assign_2nd_buf(&mut self, index: usize, val: Self::SndType) {
        unsafe { self.vec.assign_2nd_buf(self.reverse_index(index), val) }
    }
    #[inline]
    unsafe fn assign_bound_bufs(&mut self, index: usize, val: Self::BoundTypes) {
        unsafe { self.vec.assign_bound_bufs(self.reverse_index(index), val) }
    }
    #[inline]
    unsafe fn get_1st_buffer(&mut self) -> Self::FstOwnedBuffer {
        unsafe { self.vec.get_1st_buffer() }
    ...
    #[inline]
    unsafe fn drop_1st_buf_index(&mut self, index: usize) {
        unsafe { self.vec.drop_1st_buf_index(self.reverse_index(index)) }
    }
    #[inline]
    unsafe fn drop_2nd_buf_index(&mut self, index: usize) {
        unsafe { self.vec.drop_2nd_buf_index(self.reverse_index(index)) }
    }
    #[inline]
    unsafe fn drop_bound_bufs_index(&mut self, index: usize) {
        unsafe { self.vec.drop_bound_bufs_index(self.reverse_index(index)) }
    }
}
