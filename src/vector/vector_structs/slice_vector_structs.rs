...
use crate::{
    ...
    util_traits::*,
    vector::{RSMathVector, RSVectorExpr, vec_util_traits::*},
};
use std::{
...
pub struct VecAttachSlice<'a, V: VectorLike<FstHandleBool = N>, T> {
    pub(crate) vec: V,
...
unsafe impl<'a, V: IsRepeatable + VectorLike<FstHandleBool = N>, T> IsRepeatable
    for VecAttachSlice<'a, V, T>
{
}
...
pub struct VecCreateSlice<V: VectorLike<FstHandleBool = N>, T> {
    pub(crate) vec: V,
...
unsafe impl<V: IsRepeatable + VectorLike<FstHandleBool = N>, T> IsRepeatable
    for VecCreateSlice<V, T>
{
}
...
pub struct VecMaybeCreateSlice<V: VectorLike, T>
where
    <V::FstHandleBool as TyBool>::Neg: Filter,
{
    pub(crate) vec: V,
    pub(crate) buf: ManuallyDrop<
        Box<[MaybeUninit<<<V::FstHandleBool as TyBool>::Neg as Filter>::Filtered<T>>]>,
    >,
}

unsafe impl<V: VectorLike, T> Get for VecMaybeCreateSlice<V, T>
where
    <V::FstHandleBool as TyBool>::Neg: Filter,
{
    type GetBool = V::GetBool;
...
unsafe impl<V: IsRepeatable + VectorLike, T> IsRepeatable for VecMaybeCreateSlice<V, T> where
    <V::FstHandleBool as TyBool>::Neg: Filter
{
}

impl<V: VectorLike, T> HasOutput for VecMaybeCreateSlice<V, T>
where
    <V::FstHandleBool as TyBool>::Neg: Filter,
{
    type OutputBool = V::OutputBool;
    ...
    (V::FstOwnedBufferBool, <V::FstHandleBool as TyBool>::Neg): TyBoolPair,
{
    ...
    type FstOwnedBuffer =
        <(V::FstHandleBool, <V::FstHandleBool as TyBool>::Neg) as SelectPair>::Selected<
            V::FstOwnedBuffer,
            RSMathVector<<<V::FstHandleBool as TyBool>::Neg as Filter>::Filtered<T>>,
        >;
    type SndOwnedBuffer = V::SndOwnedBuffer;
