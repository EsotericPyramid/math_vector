...
impl<M: MatrixLike, V: VectorLike + IsRepeatable, B: VectorBuilder, O> HasOutput
    for MatVecMul<M, V, B, O>
where
    (
        <MatRowVectorExprs<M> as HasOutput>::OutputBool,
        V::OutputBool,
    ): FilterPair,
{
    type OutputBool = <(
    ...
    unsafe fn output(&mut self) -> Self::Output {
        unsafe {
            <(
                <MatRowVectorExprs<M> as HasOutput>::OutputBool,
                V::OutputBool,
            ) as FilterPair>::filter(self.mat.output(), self.vec.output())
        }
    }
...
impl<M: MatrixLike, V: VectorLike + IsRepeatable, B: VectorBuilder, O> HasReuseBuf
    for MatVecMul<M, V, B, O>
{
...
impl<V: VectorLike + IsRepeatable, M: MatrixLike, B: VectorBuilder, O> HasOutput
    for VecMatMul<V, M, B, O>
where
    (
        V::OutputBool,
        <MatColVectorExprs<M> as HasOutput>::OutputBool,
    ): FilterPair,
{
    type OutputBool = <(
        V::OutputBool,
    ...
    unsafe fn output(&mut self) -> Self::Output {
        unsafe {
            <(
                V::OutputBool,
                <MatColVectorExprs<M> as HasOutput>::OutputBool,
            ) as FilterPair>::filter(self.vec.output(), self.mat.output())
        }
    }
...
impl<V: VectorLike + IsRepeatable, M: MatrixLike, B: VectorBuilder, O> HasReuseBuf
    for VecMatMul<V, M, B, O>
{
    type FstHandleBool = V::FstHandleBool;
    type SndHandleBool = V::SndHandleBool;
