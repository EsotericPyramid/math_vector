...
pub struct VecBufDiscard<V: VectorLike> {
    pub(crate) vec: V
}

unsafe impl<V: VectorLike> Get for VecBufDiscard<V> {
    type GetBool = V::GetBool;
    type Inputs = V::Inputs;
    type Item = V::Item;
    type BoundItems = V::BoundItems;
...
unsafe impl<V: VectorLike + IsRepeatable> IsRepeatable for VecBufDiscard<V> {}

impl<V: VectorLike> HasOutput for VecBufDiscard<V> {
    type OutputBool = V::OutputBool;
    type Output = V::Output;
...
impl<V: VectorLike> HasReuseBuf for VecBufDiscard<V> {
...
impl<V: VectorLike> Drop for VecBufDiscard<V> {
    fn drop(&mut self) {
        unsafe { self.vec.drop_1st_buffer(); }
    }
}
...
pub struct VecAttachUsedVec<V: VectorLike, USEDV: VectorLike> {
    pub(crate) vec: V,
    pub(crate) used_vec: USEDV,
...
where
    (V::OutputBool, USEDV::OutputBool): FilterPair,
{
    type OutputBool = <(V::OutputBool, USEDV::OutputBool) as TyBoolPair>::Or;
    type Output =
        <(V::OutputBool, USEDV::OutputBool) as FilterPair>::Filtered<V::Output, USEDV::Output>;
    ...
    (V::BoundHandlesBool, USEDV::BoundHandlesBool): FilterPair,
{
    ...
    type FstOwnedBufferBool =
        <(V::FstOwnedBufferBool, USEDV::FstOwnedBufferBool) as TyBoolPair>::Xor;
    type SndOwnedBufferBool =
        <(V::SndOwnedBufferBool, USEDV::SndOwnedBufferBool) as TyBoolPair>::Xor;
    type FstOwnedBuffer =
        <(V::FstOwnedBufferBool, USEDV::FstOwnedBufferBool) as SelectPair>::Selected<
            V::FstOwnedBuffer,
            USEDV::FstOwnedBuffer,
        >;
    type SndOwnedBuffer =
        <(V::SndOwnedBufferBool, USEDV::SndOwnedBufferBool) as SelectPair>::Selected<
            V::SndOwnedBuffer,
            USEDV::SndOwnedBuffer,
        >;
    type FstType = <(V::FstHandleBool, USEDV::FstHandleBool) as SelectPair>::Selected<
        V::FstType,
        USEDV::FstType,
    >;
    type SndType = <(V::SndHandleBool, USEDV::SndHandleBool) as SelectPair>::Selected<
        V::SndType,
        USEDV::SndType,
    >;
    type BoundTypes = V::BoundTypes;
    ...
    unsafe fn get_1st_buffer(&mut self) -> Self::FstOwnedBuffer {
        unsafe {
            <(V::FstOwnedBufferBool, USEDV::FstOwnedBufferBool) as SelectPair>::select(
                self.vec.get_1st_buffer(),
                self.used_vec.get_1st_buffer(),
            )
        }
    }
    #[inline]
    unsafe fn get_2nd_buffer(&mut self) -> Self::SndOwnedBuffer {
        unsafe {
            <(V::SndOwnedBufferBool, USEDV::SndOwnedBufferBool) as SelectPair>::select(
                self.vec.get_2nd_buffer(),
                self.used_vec.get_2nd_buffer(),
            )
        }
    }
    #[inline]
...
pub struct DynamicVectorLike<V: VectorLike> {
    pub(crate) vec: V,
    pub(crate) inputs: Option<V::Inputs>,
}
...
unsafe impl<V: VectorLike> Get for DynamicVectorLike<V> {
    ...
    unsafe fn get_inputs(&mut self, index: usize) -> Self::Inputs {
        unsafe {
            self.inputs = Some(self.vec.get_inputs(index));
        }
    }
    #[inline]
    unsafe fn drop_inputs(&mut self, index: usize) {
    ...
    fn process(&mut self, index: usize, _: Self::Inputs) -> (Self::Item, Self::BoundItems) {
        self.vec.process(index, self.inputs.take().unwrap())
    }
}
