...
unsafe impl<M: VectorLike<Item = V>, V: VectorLike, Wrap: MatrixBuilder> Get
    for MatColWrapper<M, V, Wrap>
{
    ...
    unsafe fn get_inputs(&mut self, index: usize) -> Self::Inputs {
        unsafe { self.mat.get_inputs(index) }
    }
    #[inline]
    unsafe fn drop_inputs(&mut self, index: usize) {
        unsafe {
            self.mat.drop_inputs(index);
        }
    }
    #[inline]
...
unsafe impl<M: VectorLike<Item = V> + IsRepeatable, V: VectorLike, Wrap: MatrixBuilder> IsRepeatable
    for MatColWrapper<M, V, Wrap>
{
}

impl<M: VectorLike<Item = V>, V: VectorLike, Wrap: MatrixBuilder> HasOutput
    for MatColWrapper<M, V, Wrap>
{
...
impl<M: VectorLike<Item = V>, V: VectorLike, Wrap: MatrixBuilder> HasReuseBuf
    for MatColWrapper<M, V, Wrap>
{
    ...
    unsafe fn drop_1st_buf_index(&mut self, index: usize) {
        unsafe { self.mat.drop_1st_buf_index(index) }
    }
    #[inline]
    unsafe fn drop_2nd_buf_index(&mut self, index: usize) {
        unsafe { self.mat.drop_2nd_buf_index(index) }
    }
    #[inline]
    unsafe fn drop_bound_bufs_index(&mut self, index: usize) {
        unsafe { self.mat.drop_bound_bufs_index(index) }
    }
}
...
unsafe impl<M: MatrixLike> Get for MatrixColumn<M> {
    ...
    #[inline]
    unsafe fn get_inputs(&mut self, index: usize) -> Self::Inputs {
        unsafe { (*self.mat).get_inputs(self.column_num, index) }
    }

    #[inline]
    unsafe fn drop_inputs(&mut self, index: usize) {
        unsafe { (*self.mat).drop_inputs(self.column_num, index) }
    }

    #[inline]
    fn process(&mut self, index: usize, inputs: Self::Inputs) -> (Self::Item, Self::BoundItems) {
        unsafe { (*self.mat).process(self.column_num, index, inputs) }
    }
}
...
impl<M: MatrixLike> HasReuseBuf for MatrixColumn<M> {
    ...
    #[inline]
    unsafe fn assign_bound_bufs(&mut self, index: usize, val: Self::BoundTypes) {
        unsafe { (*self.mat).assign_bound_bufs(self.column_num, index, val) }
    }
    #[inline]
    unsafe fn get_1st_buffer(&mut self) -> Self::FstOwnedBuffer {}
    #[inline]
    ...
    #[inline]
    unsafe fn drop_bound_bufs_index(&mut self, index: usize) {
        unsafe { (*self.mat).drop_bound_bufs_index(self.column_num, index) }
    }
}
...
unsafe impl<M: MatrixLike> Get for MatrixRow<M> {
    ...
    #[inline]
    unsafe fn get_inputs(&mut self, index: usize) -> Self::Inputs {
        unsafe { (*self.mat).get_inputs(index, self.row_num) }
    }

    #[inline]
    unsafe fn drop_inputs(&mut self, index: usize) {
        unsafe { (*self.mat).drop_inputs(index, self.row_num) }
    }

    #[inline]
    fn process(&mut self, index: usize, inputs: Self::Inputs) -> (Self::Item, Self::BoundItems) {
        unsafe { (*self.mat).process(index, self.row_num, inputs) }
    }
}
...
impl<M: MatrixLike> HasReuseBuf for MatrixRow<M> {
    ...
    #[inline]
    unsafe fn assign_bound_bufs(&mut self, index: usize, val: Self::BoundTypes) {
        unsafe { (*self.mat).assign_bound_bufs(index, self.row_num, val) }
    }
    #[inline]
    unsafe fn get_1st_buffer(&mut self) -> Self::FstOwnedBuffer {}
    #[inline]
    ...
    #[inline]
    unsafe fn drop_bound_bufs_index(&mut self, index: usize) {
        unsafe { (*self.mat).drop_bound_bufs_index(index, self.row_num) }
    }
}
