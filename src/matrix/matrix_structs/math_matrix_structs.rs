...
pub struct FullMatMul<M1: MatrixLike + Is2DRepeatable, M2: MatrixLike + Is2DRepeatable> {
    pub(crate) l_mat: M1,
    pub(crate) r_mat: M2,
    pub(crate) shared_size: usize,
}

unsafe impl<M1: MatrixLike + Is2DRepeatable, M2: MatrixLike + Is2DRepeatable> Get2D
    for FullMatMul<M1, M2>
where
    M1::Item: Mul<M2::Item>,
...
{
    type GetBool = Y;
    type AreInputsTransposed =
        <(M1::AreInputsTransposed, M2::AreInputsTransposed) as TyBoolPair>::Or;
    type Inputs = (usize, usize);
    type Item = <M1::Item as Mul<M2::Item>>::Output;
    type BoundItems = <(M1::BoundHandlesBool, M2::BoundHandlesBool) as FilterPair>::Filtered<
        M1::BoundItems,
        M2::BoundItems,
    >;
...
impl<M1: MatrixLike + Is2DRepeatable, M2: MatrixLike + Is2DRepeatable> HasOutput
    for FullMatMul<M1, M2>
where
    (M1::OutputBool, M2::OutputBool): FilterPair,
{
    type OutputBool = <(M1::OutputBool, M2::OutputBool) as TyBoolPair>::Or;
    type Output =
        <(M1::OutputBool, M2::OutputBool) as FilterPair>::Filtered<M1::Output, M2::Output>;
...
impl<M1: MatrixLike + Is2DRepeatable, M2: MatrixLike + Is2DRepeatable> Has2DReuseBuf
    for FullMatMul<M1, M2>
where
    ...
    (M1::AreBoundBuffersTransposed, M2::AreBoundBuffersTransposed): TyBoolPair,
{
    ...
    type IsFstBufferTransposed =
        <(M1::IsFstBufferTransposed, M2::IsFstBufferTransposed) as TyBoolPair>::Xor;
    type IsSndBufferTransposed =
        <(M1::IsSndBufferTransposed, M2::IsSndBufferTransposed) as TyBoolPair>::Xor;
    type AreBoundBuffersTransposed =
        <(M1::AreBoundBuffersTransposed, M2::AreBoundBuffersTransposed) as TyBoolPair>::Xor;
    type FstOwnedBuffer =
        <(M1::FstOwnedBufferBool, M2::FstOwnedBufferBool) as SelectPair>::Selected<
            M1::FstOwnedBuffer,
            M2::FstOwnedBuffer,
        >;
    type SndOwnedBuffer =
        <(M1::SndOwnedBufferBool, M2::SndOwnedBufferBool) as SelectPair>::Selected<
            M1::SndOwnedBuffer,
            M2::SndOwnedBuffer,
        >;
    type FstType =
        <(M1::FstHandleBool, M2::FstHandleBool) as SelectPair>::Selected<M1::FstType, M2::FstType>;
    type SndType =
        <(M1::SndHandleBool, M2::SndHandleBool) as SelectPair>::Selected<M1::SndType, M2::SndType>;
    type BoundTypes = <(M1::BoundHandlesBool, M2::BoundHandlesBool) as FilterPair>::Filtered<
        M1::BoundTypes,
        M2::BoundTypes,
    >;
    ...
    unsafe fn assign_1st_buf(&mut self, col_index: usize, row_index: usize, val: Self::FstType) {
        unsafe {
            let (l_val, r_val) =
                <(M1::FstHandleBool, M2::FstHandleBool) as SelectPair>::deselect(val);
    ...
    unsafe fn assign_2nd_buf(&mut self, col_index: usize, row_index: usize, val: Self::SndType) {
        unsafe {
            let (l_val, r_val) =
                <(M1::SndHandleBool, M2::SndHandleBool) as SelectPair>::deselect(val);
    ...
    unsafe fn assign_bound_bufs(
        ...
        unsafe {
            let (l_val, r_val) =
                <(M1::BoundHandlesBool, M2::BoundHandlesBool) as FilterPair>::defilter(val);
    ...
    unsafe fn get_1st_buffer(&mut self) -> Self::FstOwnedBuffer {
        unsafe {
            <(M1::FstOwnedBufferBool, M2::FstOwnedBufferBool) as SelectPair>::select(
                self.l_mat.get_1st_buffer(),
                self.r_mat.get_1st_buffer(),
            )
        }
    }
    #[inline]
    unsafe fn get_2nd_buffer(&mut self) -> Self::SndOwnedBuffer {
        unsafe {
            <(M1::SndOwnedBufferBool, M2::SndOwnedBufferBool) as SelectPair>::select(
                self.l_mat.get_2nd_buffer(),
                self.r_mat.get_2nd_buffer(),
            )
        }
    }
    #[inline]
