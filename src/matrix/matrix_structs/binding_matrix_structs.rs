...
/// struct binding the item to the buffer in first slot (and adding it to output if owned)
pub struct MatRawBind<M: MatrixLike<FstHandleBool = Y>>{pub(crate) mat: M}

unsafe impl<M: MatrixLike<FstHandleBool = Y>> Get2D for MatRawBind<M> where (M::BoundHandlesBool, Y): FilterPair {
    type GetBool = N;
    type AreInputsTransposed = M::AreInputsTransposed;
    type Inputs = M::Inputs;
    type Item = ();
...
impl<M: MatrixLike<FstHandleBool = Y>> HasOutput for MatRawBind<M> where (M::OutputBool, M::FstOwnedBufferBool): TyBoolPair {
    type OutputBool = <(M::OutputBool, M::FstOwnedBufferBool) as TyBoolPair>::Or;
    type Output = (M::Output, M::FstOwnedBuffer);

    #[inline]
    unsafe fn output(&mut self) -> Self::Output { unsafe {
        (self.mat.output(), self.mat.get_1st_buffer())
    }}
...
impl<M: MatrixLike<FstHandleBool = Y>> Has2DReuseBuf for MatRawBind<M> where (M::BoundHandlesBool, Y): FilterPair, (M::IsFstBufferTransposed, M::AreBoundBuffersTransposed): TyBoolPair {
    type FstHandleBool = N;
    type SndHandleBool = M::SndHandleBool;
    type BoundHandlesBool = <(M::BoundHandlesBool, Y) as TyBoolPair>::Or;
    type FstOwnedBufferBool = N;
