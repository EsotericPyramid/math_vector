...
pub struct MatBind<M: MatrixLike<FstHandleBool = Y>> {
    pub(crate) mat: M,
}

unsafe impl<M: MatrixLike<FstHandleBool = Y>> Get2D for MatBind<M>
where
...
impl<M: MatrixLike<FstHandleBool = Y>> HasOutput for MatBind<M>
where
    (M::OutputBool, M::FstOwnedBufferBool): FilterPair,
{
    type OutputBool = <(M::OutputBool, M::FstOwnedBufferBool) as TyBoolPair>::Or;
    type Output = <(M::OutputBool, M::FstOwnedBufferBool) as FilterPair>::Filtered<
        M::Output,
        M::FstOwnedBuffer,
    >;
    ...
    unsafe fn output(&mut self) -> Self::Output {
        unsafe {
            <(M::OutputBool, M::FstOwnedBufferBool) as FilterPair>::filter(
                self.mat.output(),
...
impl<M: MatrixLike<FstHandleBool = Y>> Has2DReuseBuf for MatBind<M>
where
    (M::BoundHandlesBool, Y): FilterPair,
    (M::IsFstBufferTransposed, M::AreBoundBuffersTransposed): TyBoolPair,
{
    ...
    type AreBoundBuffersTransposed =
        <(M::IsFstBufferTransposed, M::AreBoundBuffersTransposed) as TyBoolPair>::And;
    type FstOwnedBuffer = ();
    ...
    unsafe fn assign_bound_bufs(
        ...
        unsafe {
            let (bounded_vals, new_bound_val) =
                <(M::BoundHandlesBool, Y) as FilterPair>::defilter(val);
            self.mat
                .assign_bound_bufs(col_index, row_index, bounded_vals);
...
/// struct binding the item to the buffer in first slot (and adding it to output if owned)
pub struct MatRawBind<M: MatrixLike<FstHandleBool = Y>> {
    pub(crate) mat: M,
}

unsafe impl<M: MatrixLike<FstHandleBool = Y>> Get2D for MatRawBind<M>
where
    (M::BoundHandlesBool, Y): FilterPair,
{
    type GetBool = N;
...
impl<M: MatrixLike<FstHandleBool = Y>> HasOutput for MatRawBind<M>
where
    (M::OutputBool, M::FstOwnedBufferBool): TyBoolPair,
{
    type OutputBool = <(M::OutputBool, M::FstOwnedBufferBool) as TyBoolPair>::Or;
    type Output = (M::Output, M::FstOwnedBuffer);

    #[inline]
    unsafe fn output(&mut self) -> Self::Output {
        unsafe { (self.mat.output(), self.mat.get_1st_buffer()) }
    }
...
impl<M: MatrixLike<FstHandleBool = Y>> Has2DReuseBuf for MatRawBind<M>
where
    (M::BoundHandlesBool, Y): FilterPair,
    (M::IsFstBufferTransposed, M::AreBoundBuffersTransposed): TyBoolPair,
{
...
pub struct MatMapBind<M: MatrixLike<FstHandleBool = Y>, F: FnMut(M::Item) -> (I, B), I, B> {
    pub(crate) mat: M,
    pub(crate) f: F,
}

unsafe impl<M: MatrixLike<FstHandleBool = Y>, F: FnMut(M::Item) -> (I, B), I, B> Get2D
    for MatMapBind<M, F, I, B>
where
    (M::BoundHandlesBool, Y): FilterPair,
{
    type GetBool = Y;
    ...
    fn process(
        ...
        (
            processed_item,
            <(M::BoundHandlesBool, Y) as FilterPair>::filter(bound, processed_bound),
        )
    }
}

impl<M: MatrixLike<FstHandleBool = Y>, F: FnMut(M::Item) -> (I, B), I, B> HasOutput
    for MatMapBind<M, F, I, B>
where
    (M::OutputBool, M::FstOwnedBufferBool): FilterPair,
{
...
impl<M: MatrixLike<FstHandleBool = Y>, F: FnMut(M::Item) -> (I, B), I, B> Has2DReuseBuf
    for MatMapBind<M, F, I, B>
where
    (M::BoundHandlesBool, Y): FilterPair,
    (M::IsFstBufferTransposed, M::AreBoundBuffersTransposed): TyBoolPair,
...
pub struct MatHalfBind<M: MatrixLike<FstHandleBool = Y>> {
    pub(crate) mat: M,
}

impl<M: MatrixLike<FstHandleBool = Y>> MatHalfBind<M> {
    pub(crate) unsafe fn get_bound_buf(&mut self) -> M::FstOwnedBuffer {
        unsafe { self.mat.get_1st_buffer() }
    }
}

unsafe impl<M: MatrixLike<FstHandleBool = Y>> Get2D for MatHalfBind<M>
where
    (M::BoundHandlesBool, Y): FilterPair,
{
    type GetBool = N;
...
impl<M: MatrixLike<FstHandleBool = Y>> HasOutput for MatHalfBind<M>
where
    (M::OutputBool, M::FstOwnedBufferBool): TyBoolPair,
{
    type OutputBool = <(M::OutputBool, M::FstOwnedBufferBool) as TyBoolPair>::Or;
...
impl<M: MatrixLike<FstHandleBool = Y>> Has2DReuseBuf for MatHalfBind<M>
where
    (M::BoundHandlesBool, Y): FilterPair,
    (M::IsFstBufferTransposed, M::AreBoundBuffersTransposed): TyBoolPair,
{
