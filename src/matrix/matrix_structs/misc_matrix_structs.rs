...
where
    (M::OutputBool, USEDM::OutputBool): FilterPair,
{
    type OutputBool = <(M::OutputBool, USEDM::OutputBool) as TyBoolPair>::Or;
    type Output =
        <(M::OutputBool, USEDM::OutputBool) as FilterPair>::Filtered<M::Output, USEDM::Output>;
...
where
    ...
    (
        M::AreBoundBuffersTransposed,
        USEDM::AreBoundBuffersTransposed,
    ): TyBoolPair,
{
    ...
    type FstOwnedBufferBool =
        <(M::FstOwnedBufferBool, USEDM::FstOwnedBufferBool) as TyBoolPair>::Xor;
    type SndOwnedBufferBool =
        <(M::SndOwnedBufferBool, USEDM::SndOwnedBufferBool) as TyBoolPair>::Xor;
    type IsFstBufferTransposed =
        <(M::IsFstBufferTransposed, USEDM::IsFstBufferTransposed) as TyBoolPair>::Or;
    type IsSndBufferTransposed =
        <(M::IsSndBufferTransposed, USEDM::IsSndBufferTransposed) as TyBoolPair>::Or;
    type AreBoundBuffersTransposed = <(
        M::AreBoundBuffersTransposed,
        USEDM::AreBoundBuffersTransposed,
    ) as TyBoolPair>::And;
    type FstOwnedBuffer =
        <(M::FstOwnedBufferBool, USEDM::FstOwnedBufferBool) as SelectPair>::Selected<
            M::FstOwnedBuffer,
            USEDM::FstOwnedBuffer,
        >;
    type SndOwnedBuffer =
        <(M::SndOwnedBufferBool, USEDM::SndOwnedBufferBool) as SelectPair>::Selected<
            M::SndOwnedBuffer,
            USEDM::SndOwnedBuffer,
        >;
    type FstType = <(M::FstHandleBool, USEDM::FstHandleBool) as SelectPair>::Selected<
        M::FstType,
        USEDM::FstType,
    >;
    type SndType = <(M::SndHandleBool, USEDM::SndHandleBool) as SelectPair>::Selected<
        M::SndType,
        USEDM::SndType,
    >;
    type BoundTypes = M::BoundTypes;
    ...
    unsafe fn assign_1st_buf(
        ...
        unsafe {
            let (l_val, r_val) =
                <(M::FstHandleBool, USEDM::FstHandleBool) as SelectPair>::deselect(val);
    ...
    unsafe fn assign_2nd_buf(
        ...
        unsafe {
            let (l_val, r_val) =
                <(M::SndHandleBool, USEDM::SndHandleBool) as SelectPair>::deselect(val);
    ...
    unsafe fn get_1st_buffer(&mut self) -> Self::FstOwnedBuffer {
        unsafe {
            <(M::FstOwnedBufferBool, USEDM::FstOwnedBufferBool) as SelectPair>::select(
                self.mat.get_1st_buffer(),
                self.used_mat.get_1st_buffer(),
            )
        }
    }
    #[inline]
    unsafe fn get_2nd_buffer(&mut self) -> Self::SndOwnedBuffer {
        unsafe {
            <(M::SndOwnedBufferBool, USEDM::SndOwnedBufferBool) as SelectPair>::select(
                self.mat.get_2nd_buffer(),
                self.used_mat.get_2nd_buffer(),
            )
        }
    }
    #[inline]
...
pub struct DynamicMatrixLike<M: MatrixLike> {
    pub(crate) mat: M,
    pub(crate) inputs: Option<M::Inputs>,
}
...
unsafe impl<M: MatrixLike> Get2D for DynamicMatrixLike<M> {
    ...
    #[inline]
    unsafe fn get_inputs(&mut self, col_index: usize, row_index: usize) -> Self::Inputs {
        self.inputs = Some(unsafe { self.mat.get_inputs(col_index, row_index) })
    }
    #[inline]
    unsafe fn drop_inputs(&mut self, col_index: usize, row_index: usize) {
        unsafe { self.mat.drop_inputs(col_index, row_index) }
    ...
    fn process(
        ...
        self.mat
            .process(col_index, row_index, self.inputs.take().unwrap())
    }
}
