...
impl<T: IntoIterator,const D: usize> RowVec<T,D> {
    pub fn unwrap(self) -> T {
        self.0
    }
    ...
    pub fn transpose(self) -> ColumnVec<T,D> {
        ColumnVec(self.0)
    }
}

impl<T,const D: usize> From<[T; D]> for RowVec<[T; D],D> {
    #[inline]
    fn from(value: [T; D]) -> Self {
        RowVec(value)
    }
...
impl<T,const D: usize> TryFrom<Vec<T>> for RowVec<Vec<T>,D> {
    type Error = ();
...
//CHECK
impl<Idx,T: IntoIterator + std::ops::Index<Idx>,const D: usize> std::ops::Index<Idx> for RowVec<T,D> where T::Output: Sized {
    type Output = Scalar<T::Output>;
...
#[derive(Clone)]
pub struct RowVecIter<I: Iterator>(I);

impl<I: Iterator> Iterator for RowVecIter<I> {
    type Item = Scalar<I::Item>;
...
impl<T: IntoIterator,const D: usize> IntoIterator for RowVec<T,D> {
    type IntoIter = RowVecIter<T::IntoIter>;
    type Item = Scalar<T::Item>;
    
    #[inline]
    fn into_iter(self) -> Self::IntoIter {
        RowVecIter(self.0.into_iter())
    }
}

impl<'a,T: IntoIterator,const D: usize> IntoIterator for &'a RowVec<T,D> where &'a T: IntoIterator {
    type IntoIter = RowVecIter<<&'a T as IntoIterator>::IntoIter>;
    type Item = Scalar<<&'a T as IntoIterator>::Item>;
...
impl<'a,T: IntoIterator,const D: usize> IntoIterator for &'a mut RowVec<T,D> where &'a mut T: IntoIterator {
    type IntoIter = RowVecIter<<&'a mut T as IntoIterator>::IntoIter>;
    type Item = Scalar<<&'a mut T as IntoIterator>::Item>;

    #[inline]
    fn into_iter(self) -> Self::IntoIter {
        RowVecIter((&mut self.0).into_iter())
    }
}
...
pub mod row_vec_iterators {
    use std::ops::*;
    use super::*;

    pub struct EvalRowVec<T,const D: usize>(pub(super) T);

    impl<I: Iterator,const D: usize> IntoIterator for EvalRowVec<I,D> {
        type IntoIter = RowVecIter<I>;
        type Item = Scalar<I::Item>;
    ...
    pub trait EvalsToRowVec<const D: usize> {
        type Item;
        
        fn eval(self) -> RowVec<Vec<Self::Item>,D>;

        fn leaky_eval_array(self) -> RowVec<[Self::Item; D],D>;

        fn eval_array(self) -> RowVec<[Self::Item; D],D>;
    }

    impl<T: EvalsToRowVec<D>,const D: usize> EvalsToRowVec<D> for EvalRowVec<T,D> {
        type Item = T::Item;
    ...
    #[macro_export]
    macro_rules! impl_sync_evals_to_row_vec {
        ($($i:ident)*,$($i_buf:ident)*) => {
            impl<$($i: Iterator),*,const D: usize> SyncEvalsToRowVec for ($(EvalRowVec<$i,D>),*) {
                type EvalOut = ($(RowVec<Vec<$i::Item>,D>),*);
                type EvalArrayOut = ($(RowVec<[$i::Item; D],D>),*);
    
                #[allow(non_snake_case)]
                #[inline]
                fn sync_eval(self) -> Self::EvalOut {
                    let ($(EvalRowVec(mut $i)),*) = self;
                    $(
                        let mut $i_buf = Vec::with_capacity(D);
                    )*
                    for _ in 0..D {
                        $(
                            $i_buf.push($i.next().expect("internal_error: The first D elements in a EvalRowVec must be valid"));
                        )*
                    }
                    ($(RowVec($i_buf)),*)
                }
                ...
                fn sync_leaky_eval_array(self) -> Self::EvalArrayOut {
                    ...
                    for i in 0..D {
                        $(
                            $i_buf[i] = std::mem::MaybeUninit::new($i.next().expect("internal_error: The first D elements in a EvalRowVec must be valid"));
                        )*
                    }
                    ($(RowVec(unsafe {
                        let initialized_arr: [$i::Item; D] = std::ptr::read(std::mem::transmute(&$i_buf as *const [std::mem::MaybeUninit<$i::Item>; D]));
                ...
                fn sync_eval_array(self) -> Self::EvalArrayOut {
                    ...
                    ($(RowVec(unsafe {$i_buf.assume_init()})),*)
                }
            }
        };
    }
    
    macro_rules! recursive_impl_sync_evals_to_row_vec {
        ($i:ident,$i_buf:ident) => {};
        ($i_first:ident $($i:ident)* ,$i_buf_first:ident $($i_buf:ident)* ) => {
            impl_sync_evals_to_row_vec!($i_first $($i)*,$i_buf_first $($i_buf)*);
            recursive_impl_sync_evals_to_row_vec!($($i)*,$($i_buf)*);
        };
    } 

    recursive_impl_sync_evals_to_row_vec!(A B C NotD E F G H I J K L M N O P,A_buf B_buf C_buf D_buf E_buf F_buf G_buf H_buf I_buf J_buf K_buf L_buf M_buf N_buf O_buf P_buf);


    macro_rules! impl_eval_row_vec {
        ($t:ty,1,$ti:ty,$tr:path) => {
            impl<I: Iterator,S: Copy,const D: usize> EvalsToRowVec<D> for $t where $ti: $tr {
                type Item = <$t as Iterator>::Item;
                ...
                #[inline]
                fn leaky_eval_array(self) -> RowVec<[Self::Item; D],D> {
                    RowVec(unsafe { utils::leaky_iterator_to_array(self) })
                }

                #[inline]
                fn eval_array(self) -> RowVec<[Self::Item; D],D> {
                    RowVec(unsafe { utils::iterator_to_array(self) })
                }
            }
        };
        ($t:ty,2,$ti:ty,$tr:path) => {
            impl<I1: Iterator,I2: Iterator,const D: usize> EvalsToRowVec<D> for $t where $ti: $tr {
                type Item = <$t as Iterator>::Item;
    ...
    macro_rules! overload_given_operator_for_vec {
        ($Op:ident,$OpF:ident,$OpI:ident) => {
            impl<I1: IntoIterator,I2: IntoIterator,const D: usize> $Op<RowVec<I2,D>> for RowVec<I1,D> where I1::Item: $Op<I2::Item> {
                type Output = EvalRowVec<$OpI<I1::IntoIter,I2::IntoIter,D>,D>;

                #[inline]
                fn $OpF(self,other: RowVec<I2,D>) -> Self::Output {
                    EvalRowVec($OpI(
                        self.0.into_iter(),
                        other.0.into_iter()
                    ))
            ...
            impl<'a,I1: IntoIterator,I2: IntoIterator,const D: usize> $Op<RowVec<I2,D>> for &'a RowVec<I1,D> 
            where &'a I1: IntoIterator, <&'a I1 as IntoIterator>::Item: $Op<I2::Item> {
                type Output = EvalRowVec<$OpI<<&'a I1 as IntoIterator>::IntoIter,I2::IntoIter,D>,D>;
            ...
            impl<'a,I1: IntoIterator,I2: IntoIterator,const D: usize> $Op<&'a RowVec<I2,D>> for RowVec<I1,D> 
            where &'a I2: IntoIterator, I1::Item: $Op<<&'a I2 as IntoIterator>::Item> {
                type Output = EvalRowVec<$OpI<I1::IntoIter,<&'a I2 as IntoIterator>::IntoIter,D>,D>;

                #[inline]
                fn $OpF(self,other: &'a RowVec<I2,D>) -> Self::Output {
                    EvalRowVec($OpI(
                        self.0.into_iter(),
                        (&other.0).into_iter()
                    ))
            ...
            impl<'a,I1: IntoIterator,I2: IntoIterator,const D: usize> $Op<&'a RowVec<I2,D>> for &'a RowVec<I1,D> 
            where &'a I1: IntoIterator, &'a I2: IntoIterator, <&'a I1 as IntoIterator>::Item: $Op<<&'a I2 as IntoIterator>::Item> {
                type Output = EvalRowVec<$OpI<<&'a I1 as IntoIterator>::IntoIter,<&'a I2 as IntoIterator>::IntoIter,D>,D>;
            ...
            impl<I1: Iterator,I2: IntoIterator,const D: usize> $Op<RowVec<I2,D>> for EvalRowVec<I1,D> where I1::Item: $Op<I2::Item> {
                type Output = EvalRowVec<$OpI<I1,I2::IntoIter,D>,D>;
            ...
            impl<'a,I1: Iterator,I2: IntoIterator,const D: usize> $Op<&'a RowVec<I2,D>> for EvalRowVec<I1,D> 
            where &'a I2: IntoIterator, I1::Item: $Op<<&'a I2 as IntoIterator>::Item> {
                type Output = EvalRowVec<$OpI<I1,<&'a I2 as IntoIterator>::IntoIter,D>,D>;
            ...
            impl<I1: IntoIterator,I2: Iterator,const D: usize> $Op<EvalRowVec<I2,D>> for RowVec<I1,D> where I1::Item: $Op<I2::Item> {
                type Output = EvalRowVec<$OpI<I1::IntoIter,I2,D>,D>;

                #[inline]
                fn $OpF(self,other: EvalRowVec<I2,D>) -> Self::Output {
                    EvalRowVec($OpI(
                        self.0.into_iter(),
                        other.0
                    ))
            ...
            impl<'a,I1: IntoIterator,I2: Iterator,const D: usize> $Op<EvalRowVec<I2,D>> for &'a RowVec<I1,D> 
            where &'a I1: IntoIterator, <&'a I1 as IntoIterator>::Item: $Op<I2::Item> {
                type Output = EvalRowVec<$OpI<<&'a I1 as IntoIterator>::IntoIter,I2,D>,D>;
            ...
            impl<I1: Iterator,I2: Iterator,const D: usize> $Op<EvalRowVec<I2,D>> for EvalRowVec<I1,D> 
            where I1::Item: $Op<I2::Item> {
                type Output = EvalRowVec<$OpI<I1,I2,D>,D>;
        ...
        ($Op:ident,$OpF:ident,$OpI:ident,0) => {
            impl<I: IntoIterator,S: Copy,const D: usize> $Op<Scalar<S>> for RowVec<I,D> where I::Item: $Op<S> {
                type Output = EvalRowVec<$OpI<I::IntoIter,S,D>,D>;
            ...
            impl<'a,I: IntoIterator,S: Copy,const D: usize> $Op<Scalar<S>> for &'a RowVec<I,D> 
            where &'a I: IntoIterator, <&'a I as IntoIterator>::Item: $Op<S> {
                type Output = EvalRowVec<$OpI<<&'a I as IntoIterator>::IntoIter,S,D>,D>;
            ...
            impl<'a,I: IntoIterator,S: Copy,const D: usize> $Op<&'a Scalar<S>> for RowVec<I,D> where I::Item: $Op<&'a S> {
                type Output = EvalRowVec<$OpI<I::IntoIter,&'a S,D>,D>;
            ...
            impl<'a,I: IntoIterator,S: Copy,const D: usize> $Op<&'a Scalar<S>> for &'a RowVec<I,D> 
            where &'a I: IntoIterator, <&'a I as IntoIterator>::Item: $Op<&'a S> {
                type Output = EvalRowVec<$OpI<<&'a I as IntoIterator>::IntoIter,&'a S,D>,D>;
            ...
            impl<I: Iterator,S: Copy,const D: usize> $Op<Scalar<S>> for EvalRowVec<I,D> where I::Item: $Op<S> {
                type Output = EvalRowVec<$OpI<I,S,D>,D>;
            ...
            impl<'a,I: Iterator,S: Copy,const D: usize> $Op<&'a Scalar<S>> for EvalRowVec<I,D> where I::Item: $Op<&'a S> {
                type Output = EvalRowVec<$OpI<I,&'a S,D>,D>;
        ...
        ($Op:ident,$OpF:ident,$OpI:ident,1) => {
            impl<I: IntoIterator,S: Copy,const D: usize> $Op<RowVec<I,D>> for Scalar<S> where S: $Op<I::Item> {
                type Output = EvalRowVec<$OpI<S,I::IntoIter,D>,D>;

                #[inline]
                fn $OpF(self,rhs: RowVec<I,D>) -> Self::Output {
                    EvalRowVec($OpI(
                        rhs.0.into_iter(),
                        self.0
                    ))
            ...
            impl<'a,I: IntoIterator,S: Copy,const D: usize> $Op<RowVec<I,D>> for &'a Scalar<S> where &'a S: $Op<I::Item> {
                type Output = EvalRowVec<$OpI<&'a S,I::IntoIter,D>,D>;
            ...
            impl<'a,I: IntoIterator,S: Copy,const D: usize> $Op<&'a RowVec<I,D>> for Scalar<S> 
            where &'a I: IntoIterator, S: $Op<<&'a I as IntoIterator>::Item> {
                type Output = EvalRowVec<$OpI<S,<&'a I as IntoIterator>::IntoIter,D>,D>;

                #[inline]
                fn $OpF(self,rhs: &'a RowVec<I,D>) -> Self::Output {
                    EvalRowVec($OpI(
                        (&rhs.0).into_iter(),
                        self.0
                    ))
            ...
            impl<'a,I: IntoIterator,S: Copy,const D: usize> $Op<&'a RowVec<I,D>> for &'a Scalar<S> 
            where &'a I: IntoIterator, &'a S: $Op<<&'a I as IntoIterator>::Item> {
                type Output = EvalRowVec<$OpI<&'a S,<&'a I as IntoIterator>::IntoIter,D>,D>;
            ...
            impl<I: Iterator,S: Copy,const D: usize> $Op<EvalRowVec<I,D>> for Scalar<S> where S: $Op<I::Item> {
                type Output = EvalRowVec<$OpI<S,I,D>,D>;

                #[inline]
                fn $OpF(self,rhs: EvalRowVec<I,D>) -> Self::Output {
                    EvalRowVec($OpI(
                        rhs.0,
                        self.0
                    ))
            ...
            impl<'a, I: Iterator,S: Copy,const D: usize> $Op<EvalRowVec<I,D>> for &'a Scalar<S> where &'a S: $Op<I::Item> {
                type Output = EvalRowVec<$OpI<&'a S,I,D>,D>;
    ...
    pub struct RowVecAdd<I1: Iterator,I2: Iterator,const D: usize>(I1,I2) where I1::Item: Add<I2::Item>;

    impl<I1: Iterator,I2: Iterator,const D: usize> Iterator for RowVecAdd<I1,I2,D> where I1::Item: Add<I2::Item> {
        type Item = <I1::Item as Add<I2::Item>>::Output;
    ...
    impl_eval_row_vec!(RowVecAdd<I1,I2,D>,2,I1::Item,Add<I2::Item>);
    overload_given_operator_for_vec!(Add,add,RowVecAdd);

    pub struct RowVecSub<I1: Iterator,I2: Iterator,const D: usize>(I1,I2) where I1::Item: Sub<I2::Item>;
    
    impl<I1: Iterator,I2: Iterator,const D: usize> Iterator for RowVecSub<I1,I2,D> where I1::Item: Sub<I2::Item> {
        type Item = <I1::Item as Sub<I2::Item>>::Output;
    ...
    impl_eval_row_vec!(RowVecSub<I1,I2,D>,2,I1::Item,Sub<I2::Item>);
    overload_given_operator_for_vec!(Sub,sub,RowVecSub);

    pub struct RowVecMulAsFirst<I: Iterator,S: Copy,const D: usize>(I,S) where I::Item: Mul<S>;

    impl<I: Iterator,S: Copy,const D: usize> Iterator for RowVecMulAsFirst<I,S,D> where I::Item: Mul<S> {
        type Item = <I::Item as Mul<S>>::Output;
    ...
    impl_eval_row_vec!(RowVecMulAsFirst<I,S,D>,1,I::Item,Mul<S>);
    overload_given_operator_for_vec!(Mul,mul,RowVecMulAsFirst,0);

    pub struct RowVecMulAsSecond<S: Copy,I: Iterator,const D: usize>(I,S) where S: Mul<I::Item>;

    impl<S: Copy,I: Iterator,const D: usize> Iterator for RowVecMulAsSecond<S,I,D> where S: Mul<I::Item> {
        type Item = S::Output;
    ...
    impl_eval_row_vec!(RowVecMulAsSecond<S,I,D>,1,S,Mul<I::Item>);
    overload_given_operator_for_vec!(Mul,mul,RowVecMulAsSecond,1);

    pub struct RowVecDivAsDividend<I: Iterator,S: Copy,const D: usize>(I,S) where I::Item: Div<S>;

    impl<I: Iterator,S: Copy,const D: usize> Iterator for RowVecDivAsDividend<I,S,D> where I::Item: Div<S> {
        type Item = <I::Item as Div<S>>::Output;
    ...
    impl_eval_row_vec!(RowVecDivAsDividend<I,S,D>,1,I::Item,Div<S>);
    overload_given_operator_for_vec!(Div,div,RowVecDivAsDividend,0);

    pub struct RowVecDivAsDivisor<S: Copy,I: Iterator,const D: usize>(I,S) where S: Div<I::Item>; 

    impl<S: Copy,I: Iterator,const D: usize> Iterator for RowVecDivAsDivisor<S,I,D> where S: Div<I::Item> {
        type Item = S::Output;
    ...
    impl_eval_row_vec!(RowVecDivAsDivisor<S,I,D>,1,S,Div<I::Item>);
    overload_given_operator_for_vec!(Div,div,RowVecDivAsDivisor,1);

    pub struct RowVecRemAsDividend<I: Iterator,S: Copy,const D: usize>(I,S) where I::Item: Rem<S>;

    impl<I: Iterator,S: Copy,const D: usize> Iterator for RowVecRemAsDividend<I,S,D> where I::Item: Rem<S> {
        type Item = <I::Item as Rem<S>>::Output;
    ...
    impl_eval_row_vec!(RowVecRemAsDividend<I,S,D>,1,I::Item,Rem<S>);
    overload_given_operator_for_vec!(Rem,rem,RowVecRemAsDividend,0);

    pub struct RowVecRemAsDivisor<S: Copy,I: Iterator,const D: usize>(I,S) where S: Rem<I::Item>;

    impl<S: Copy,I: Iterator,const D: usize> Iterator for RowVecRemAsDivisor<S,I,D> where S: Rem<I::Item> {
        type Item = S::Output;
    ...
    impl_eval_row_vec!(RowVecRemAsDivisor<S,I,D>,1,S,Rem<I::Item>);
    overload_given_operator_for_vec!(Rem,rem,RowVecRemAsDivisor,1);

    //Assign operators
    macro_rules! impl_assign_operator_for_vec {
        ($Op:ident,$OpF:ident,0) => {
            impl<I1: IntoIterator,I2: IntoIterator,O,const D: usize> $Op<RowVec<I2,D>> for RowVec<I1,D> 
            where for<'a> &'a mut I1: IntoIterator<Item = &'a mut O>,O: $Op<I2::Item> {
                #[inline]
                fn $OpF(&mut self,rhs: RowVec<I2,D>) {
                    for (self_val,other_val) in (&mut self.0).into_iter().zip(rhs.0.into_iter()) {
                        (self_val).$OpF(other_val)
                    }
                }
            ...
            impl<'b,I1: IntoIterator,I2: IntoIterator,O,const D: usize> $Op<&'b RowVec<I2,D>> for RowVec<I1,D> 
            where &'b I2: IntoIterator,for<'a> &'a mut I1: IntoIterator<Item = &'a mut O>,O: $Op<<&'b I2 as IntoIterator>::Item> {
                #[inline]
                fn $OpF(&mut self,rhs: &'b RowVec<I2,D>) {
                    for (self_val,other_val) in (&mut self.0).into_iter().zip((&rhs.0).into_iter()) {
                        (self_val).$OpF(other_val)
                    }
                }
            ...
            impl<I1: IntoIterator,I2: Iterator,O,const D: usize> $Op<EvalRowVec<I2,D>> for RowVec<I1,D> 
            where for<'a> &'a mut I1: IntoIterator<Item = &'a mut O>,O: $Op<I2::Item> {
                #[inline]
                fn $OpF(&mut self,rhs: EvalRowVec<I2,D>) {
                    for (self_val,other_val) in (&mut self.0).into_iter().zip(rhs.0) {
                        (self_val).$OpF(other_val)
                    }
                }
        ...
        ($Op:ident,$OpF:ident,1) => {
            impl<I: IntoIterator,O,S: Copy,const D: usize> $Op<Scalar<S>> for RowVec<I,D> 
            where for<'a> &'a mut I: IntoIterator<Item = &'a mut O>,for<'a> O: $Op<S> {
                #[inline]
                fn $OpF(&mut self,rhs: Scalar<S>) {
                    for val in (&mut self.0).into_iter() {
            ...
            impl<'b,I: IntoIterator,O,S: Copy,const D: usize> $Op<&'b Scalar<S>> for RowVec<I,D> 
            where for<'a> &'a mut I: IntoIterator<Item = &'a mut O>,for<'a> O: $Op<&'b S> {
                #[inline]
                fn $OpF(&mut self,rhs: &'b Scalar<S>) {
                    for val in (&mut self.0).into_iter() {
    ...
    //assumes the default is something akin to 0
    impl<I1: IntoIterator,I2: IntoIterator,O: AddAssign<O> + std::default::Default ,const D: usize> Dot<RowVec<I2,D>> for RowVec<I1,D>
    where I1::Item: Mul<I2::Item,Output = O>  {
        type Output = Scalar<O>;

        #[inline]
        fn dot(self,rhs: RowVec<I2,D>) -> Self::Output {
            let mut output = <O>::default();
            for (left_val,right_val) in self.0.into_iter().zip(rhs.0.into_iter()) {
                output += left_val * right_val;
            }
    ...
    impl<'a, I1: IntoIterator,I2: IntoIterator,O: AddAssign<O> + std::default::Default ,const D: usize> Dot<RowVec<I2,D>> for &'a RowVec<I1,D>
    where &'a I1: IntoIterator,<&'a I1 as IntoIterator>::Item: Mul<I2::Item,Output = O>  {
        type Output = Scalar<O>;
    ...
    impl<'a, I1: IntoIterator,I2: IntoIterator,O: AddAssign<O> + std::default::Default ,const D: usize> Dot<&'a RowVec<I2,D>> for RowVec<I1,D>
    where &'a I2: IntoIterator, I1::Item: Mul<<&'a I2 as IntoIterator>::Item,Output = O>  {
        type Output = Scalar<O>;

        #[inline]
        fn dot(self,rhs: &'a RowVec<I2,D>) -> Self::Output {
            let mut output = <O>::default();
            for (left_val,right_val) in self.0.into_iter().zip((&rhs.0).into_iter()) {
                output += left_val * right_val;
            }
    ...
    impl<'a, I1: IntoIterator,I2: IntoIterator,O: AddAssign<O> + std::default::Default ,const D: usize> Dot<&'a RowVec<I2,D>> for &'a RowVec<I1,D>
    where &'a I2: IntoIterator, &'a I1: IntoIterator, <&'a I1 as IntoIterator>::Item: Mul<<&'a I2 as IntoIterator>::Item,Output = O>  {
        type Output = Scalar<O>;
    ...
    impl<I1: Iterator,I2: IntoIterator,O: AddAssign<O> + std::default::Default,const D: usize> Dot<RowVec<I2,D>> for EvalRowVec<I1,D> 
    where I1::Item: Mul<I2::Item,Output = O> {
        type Output = Scalar<O>;
    ...
    impl<'a,I1: Iterator,I2: IntoIterator,O: AddAssign<O> + std::default::Default,const D: usize> Dot<&'a RowVec<I2,D>> for EvalRowVec<I1,D> 
    where &'a I2: IntoIterator, I1::Item: Mul<<&'a I2 as IntoIterator>::Item,Output = O> {
        type Output = Scalar<O>;
    ...
    impl<I1: IntoIterator,I2: Iterator,O: AddAssign<O> + std::default::Default,const D: usize> Dot<EvalRowVec<I2,D>> for RowVec<I1,D> 
    where I1::Item: Mul<I2::Item,Output = O> {
        type Output = Scalar<O>;

        #[inline]
        fn dot(self,rhs: EvalRowVec<I2,D>) -> Self::Output {
            let mut output = <O>::default();
            for (left_val,right_val) in self.0.into_iter().zip(rhs.0) {
                output += left_val * right_val;
            }
    ...
    impl<'a, I1: IntoIterator,I2: Iterator,O: AddAssign<O> + std::default::Default,const D: usize> Dot<EvalRowVec<I2,D>> for &'a RowVec<I1,D> 
    where &'a I1: IntoIterator,<&'a I1 as IntoIterator>::Item: Mul<I2::Item,Output = O> {
        type Output = Scalar<O>;
    ...
    impl<I1: Iterator,I2: Iterator,O: AddAssign<O> + std::default::Default,const D: usize> Dot<EvalRowVec<I2,D>> for EvalRowVec<I1,D> 
    where I1::Item: Mul<I2::Item,Output = O> {
        type Output = Scalar<O>;
    ...
    //Cloned and Copied
    pub struct ClonedRowVec<'a,I: Iterator<Item = &'a T>,T: 'a,const D: usize>(I) where T: Clone;

    impl<'a,I: Iterator<Item = &'a T>,T: 'a,const D: usize> Iterator for ClonedRowVec<'a,I,T,D> where T: Clone {
        type Item = T;
    ...
    impl<'a,I: Iterator<Item = &'a T>,T: 'a,const D: usize> EvalsToRowVec<D> for ClonedRowVec<'a,I,T,D> where T: Clone {
        type Item = T;
    ...
    pub struct CopiedRowVec<'a,I: Iterator<Item = &'a T>,T: 'a,const D: usize>(I) where T: Copy;

    impl<'a,I: Iterator<Item = &'a T>,T: 'a,const D: usize> Iterator for CopiedRowVec<'a,I,T,D> where T: Copy {
        type Item = T;
    ...
    impl<'a,I: Iterator<Item = &'a T>,T: 'a,const D: usize> EvalsToRowVec<D> for CopiedRowVec<'a,I,T,D> where T: Copy {
        type Item = T;
    ...
    impl<'a,I: Iterator<Item = &'a T>,T: 'a,const D: usize> EvalRowVec<I,D> {
        #[inline]
        pub fn cloned(self) -> EvalRowVec<ClonedRowVec<'a,I,T,D>,D> where T: Clone{
            EvalRowVec(ClonedRowVec(
                self.0
            ))
        }

        #[inline]
        pub fn copied(self) -> EvalRowVec<CopiedRowVec<'a,I,T,D>,D> where T: Copy{
            EvalRowVec(CopiedRowVec(
                self.0
            ))
        }
    }

    impl<'a,I: IntoIterator<Item = &'a T>,T: 'a,const D: usize> RowVec<I,D> {
        #[inline]
        pub fn cloned(self) -> EvalRowVec<ClonedRowVec<'a,I::IntoIter,T,D>,D> where T: Clone{
            EvalRowVec(ClonedRowVec(
                self.0.into_iter()
            ))
        }
        ...
        #[inline]
        pub fn copied(self) -> EvalRowVec<CopiedRowVec<'a,I::IntoIter,T,D>,D> where T: Copy{
            EvalRowVec(CopiedRowVec(
                self.0.into_iter()
            ))
        }
    ...
    //Duplicate
    struct DuplicatedRowVec<I: Iterator,const D: usize> where I::Item: Clone {
        iterator: I,
        buffer: Option<Option<I::Item>>,
    }

    pub struct FirstDuplicateRowVec<I: Iterator,const D: usize>(std::rc::Rc<std::cell::RefCell<DuplicatedRowVec<I,D>>>) where I::Item: Clone;

    pub struct SecondDuplicateRowVec<I: Iterator,const D: usize>(std::rc::Rc<std::cell::RefCell<DuplicatedRowVec<I,D>>>) where I::Item: Clone;

    impl<I: Iterator,const D: usize> Iterator for FirstDuplicateRowVec<I,D> where I::Item: Clone {
        type Item = I::Item;
        ...
        fn next(&mut self) -> Option<Self::Item> {
            let val = self.0.borrow_mut().iterator.next();
            if let Some(_) = self.0.borrow().buffer {
                panic!("math_vector: Error, FirstDuplicateRowVec<...> must be read before SecondDuplicateRowVec<...> for each item within\n\nThe FirstDuplicateRowVec is likely being fully read before the other (ie finding the dot product with one)")
            }
            self.0.borrow_mut().buffer = Some(val.clone());
            val
        }
    ...
    impl<I: Iterator,const D: usize> Iterator for SecondDuplicateRowVec<I,D> where I::Item: Clone {
        type Item = I::Item;
        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            match val {
                Some(inner_val) => inner_val,
                None => {panic!("math_vector: Error, FirstDuplicateRowVec<...> must be read before SecondDuplicateRowVec<...> for each item within\n\nEither you need to switch around the order of these 2 or \none of these 2 is being fully read before the other (ie finding the dot product with one)")}
            }
        }
    }


    impl<I: Iterator,const D: usize> EvalRowVec<I,D> where I::Item: Clone {
        #[inline]
        pub fn duplicate(self) -> (EvalRowVec<FirstDuplicateRowVec<I,D>,D>,EvalRowVec<SecondDuplicateRowVec<I,D>,D>) {
            let shared_iter = std::rc::Rc::new(std::cell::RefCell::new(DuplicatedRowVec{
                iterator: self.0,
                buffer: None
            }));
            (
                EvalRowVec(FirstDuplicateRowVec(shared_iter.clone())),
                EvalRowVec(SecondDuplicateRowVec(shared_iter))
            )
        }
    }

    impl<I: IntoIterator,const D: usize> RowVec<I,D> where I::Item: Clone {
        #[inline]
        pub fn duplicate(self) -> (EvalRowVec<FirstDuplicateRowVec<I::IntoIter,D>,D>,EvalRowVec<SecondDuplicateRowVec<I::IntoIter,D>,D>) {
            let shared_iter = std::rc::Rc::new(std::cell::RefCell::new(DuplicatedRowVec{
                iterator: self.0.into_iter(),
                buffer: None
            }));
        ...
        #[inline]
        pub fn duplicate_ref<'a>(&'a self) -> (EvalRowVec<FirstDuplicateRowVec<<&'a I as IntoIterator>::IntoIter,D>,D>,EvalRowVec<SecondDuplicateRowVec<<&'a I as IntoIterator>::IntoIter,D>,D>) 
        where &'a I: IntoIterator, <&'a I as IntoIterator>::Item: Clone {
            let shared_iter = std::rc::Rc::new(std::cell::RefCell::new(DuplicatedRowVec{
                iterator: (&self.0).into_iter(),
                buffer: None
    ...
    //Unchecked Duplicate
    struct UncheckedDuplicatedRowVec<I: Iterator,const D: usize> where I::Item: Clone {
        iterator: I,
        buffer: Option<I::Item>
    }

    pub struct FirstUncheckedDuplicateRowVec<I: Iterator,const D: usize>(std::rc::Rc<utils::UncheckedCell<UncheckedDuplicatedRowVec<I,D>>>) where I::Item: Clone;

    pub struct SecondUncheckedDuplicateRowVec<I: Iterator,const D: usize>(std::rc::Rc<utils::UncheckedCell<UncheckedDuplicatedRowVec<I,D>>>) where I::Item: Clone;

    impl<I: Iterator,const D: usize> Iterator for FirstUncheckedDuplicateRowVec<I,D> where I::Item: Clone {
        type Item = I::Item;
    ...
    impl<I: Iterator,const D: usize> Iterator for SecondUncheckedDuplicateRowVec<I,D> where I::Item: Clone {
        type Item = I::Item;
    ...
    impl<I: Iterator,const D: usize> EvalRowVec<I,D> where I::Item: Clone {
        #[inline]
        pub unsafe fn unchecked_duplicate(self) -> (EvalRowVec<FirstUncheckedDuplicateRowVec<I,D>,D>,EvalRowVec<SecondUncheckedDuplicateRowVec<I,D>,D>) {
            let shared_iter = std::rc::Rc::new(utils::UncheckedCell::new(UncheckedDuplicatedRowVec{
                iterator: self.0,
                buffer: None
            }));
            (
                EvalRowVec(FirstUncheckedDuplicateRowVec(shared_iter.clone())),
                EvalRowVec(SecondUncheckedDuplicateRowVec(shared_iter))
            )
        }
    }
    ...
    impl<I: IntoIterator,const D: usize> RowVec<I,D> where I::Item: Clone {
        #[inline]
        pub unsafe fn unchecked_duplicate(self) -> (EvalRowVec<FirstUncheckedDuplicateRowVec<I::IntoIter,D>,D>,EvalRowVec<SecondUncheckedDuplicateRowVec<I::IntoIter,D>,D>) {
            let shared_iter = std::rc::Rc::new(utils::UncheckedCell::new(UncheckedDuplicatedRowVec{
                iterator: self.0.into_iter(),
                buffer: None
            }));
        ...
        #[inline]
        pub unsafe fn unchecked_duplicate_ref<'a>(&'a self) -> (EvalRowVec<FirstUncheckedDuplicateRowVec<<&'a I as IntoIterator>::IntoIter,D>,D>,EvalRowVec<SecondUncheckedDuplicateRowVec<<&'a I as IntoIterator>::IntoIter,D>,D>) 
        where &'a I: IntoIterator, <&'a I as IntoIterator>::Item: Clone {
            let shared_iter = std::rc::Rc::new(utils::UncheckedCell::new(UncheckedDuplicatedRowVec{
                iterator: (&self.0).into_iter(),
                buffer: None
    ...
    //buffer ops (clone_to_buffer,copy_to_buffer)
    pub struct BufferedRowVec<T,const D: usize>(utils::IterInitArray<T,D>);

    impl<T,const D: usize> EvalsToRowVec<D> for BufferedRowVec<T,D> {
        type Item = T;

        fn eval(self) -> RowVec<Vec<Self::Item>,D> {
            if self.0.last_index == Some(D-1) {
                RowVec(unsafe { self.0.assume_init() }.into())
            } else if self.0.last_index == Some(0) {
                panic!("A BufferedRowVec must be written to before being evaluated");
            } else {
                panic!("A BufferedRowVec must be written to only once");
            }
        }

        fn leaky_eval_array(self) -> RowVec<[Self::Item; D],D> {
            eprintln!("math_vector: Warning, BufferedRowVec's leaky_eval_array is equivilent to its eval_array. \n\tUse LeakyUncheckedBufferedRowVec for that instead");
            self.eval_array()
        }

        fn eval_array(self) -> RowVec<[Self::Item; D],D> {
            if self.0.last_index == Some(D-1) {
                RowVec(unsafe { self.0.assume_init() })
            } else if self.0.last_index == Some(0) {
                panic!("A BufferedRowVec must be written to before being evaluated");
            } else {
                panic!("A BufferedRowVec must be written to only once");
    ...
    pub fn get_row_vec_buffer<T,const D: usize>() -> EvalRowVec<BufferedRowVec<T,D>,D> {
        EvalRowVec(BufferedRowVec(utils::IterInitArray{array: unsafe {std::mem::MaybeUninit::uninit().assume_init()},last_index: None}))
    }
    
        //Clone
    pub struct CloneToBufferRowVec<'a,I: Iterator,const D: usize> where I::Item: Clone {iter: I, buffer: &'a mut BufferedRowVec<I::Item,D>}

    impl<'a,I: Iterator,const D: usize> Iterator for CloneToBufferRowVec<'a,I,D> where I::Item: Clone {
        type Item = I::Item;
        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            //No reason not to implement this but its kinda dumb
    impl<'a,I: Iterator,const D: usize> EvalsToRowVec<D> for CloneToBufferRowVec<'a,I,D> where I::Item: Clone {
        type Item = I::Item;
        ...
        //Copy
    pub struct CopyToBufferRowVec<'a,I: Iterator,const D: usize> where I::Item: Copy {iter: I, buffer: &'a mut BufferedRowVec<I::Item,D>}

    impl<'a,I: Iterator,const D: usize> Iterator for CopyToBufferRowVec<'a,I,D> where I::Item: Copy {
        type Item = I::Item;
        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            //No reason not to implement this but its kinda dumb
    impl<'a,I: Iterator,const D: usize> EvalsToRowVec<D> for CopyToBufferRowVec<'a,I,D> where I::Item: Copy {
        type Item = I::Item;
    ...
    impl<I: Iterator,const D: usize> EvalRowVec<I,D> {
        pub fn clone_to_buffer<'a>(self,buf: &'a mut EvalRowVec<BufferedRowVec<I::Item,D>,D>) -> EvalRowVec<CloneToBufferRowVec<'a,I,D>,D> where I::Item: Clone {
            EvalRowVec(CloneToBufferRowVec{
                iter: self.0,
                buffer: &mut buf.0
            })
        }

        pub fn copy_to_buffer<'a>(self,buf: &'a mut EvalRowVec<BufferedRowVec<I::Item,D>,D>) -> EvalRowVec<CopyToBufferRowVec<'a,I,D>,D> where I::Item: Copy {
            EvalRowVec(CopyToBufferRowVec{
                iter: self.0,
                buffer: &mut buf.0
            })
        }
    ...
    //Map
    pub struct RowVecMap<I: Iterator,F: FnMut(I::Item) -> O,O,const D: usize>(I,F);

    impl<I: Iterator,F: FnMut(I::Item) -> O,O,const D: usize> Iterator for RowVecMap<I,F,O,D> {
        type Item = O;
    ...
    impl<I: Iterator,F: FnMut(I::Item) -> O,O,const D: usize> EvalsToRowVec<D> for RowVecMap<I,F,O,D> {
        type Item = O;
    ...
    impl<I: Iterator,const D: usize> EvalRowVec<I,D> {
        #[inline]
        pub fn map<F: FnMut(I::Item) -> O,O>(self,f: F) -> RowVecMap<I,F,O,D> {
            RowVecMap(self.0,f)
        }
    }
    
    impl<I: IntoIterator,const D: usize> RowVec<I,D> {
        #[inline]
        pub fn map<F: FnMut(I::Item) -> O,O>(self,f: F) -> RowVecMap<I::IntoIter,F,O,D> {
            RowVecMap(self.0.into_iter(),f)
        }

        #[inline]
        pub fn map_ref<'a,F: FnMut(<&'a I as IntoIterator>::Item) -> O,O>(&'a self,f: F) -> RowVecMap<<&'a I as IntoIterator>::IntoIter,F,O,D> where &'a I: IntoIterator {
            RowVecMap((&self.0).into_iter(),f)
        }
    }
    
    //Map_and_store
    pub struct RowVecMapAndStore<'a,I: Iterator,F,O,const D: usize> where for<'b> F: FnMut(&'b I::Item) -> O {iter: I, f: F, buffer: &'a mut BufferedRowVec<I::Item,D> }

    impl<'a,I: Iterator,F,O,const D: usize> Iterator for RowVecMapAndStore<'a,I,F,O,D> where for<'b> F: FnMut(&'b I::Item) -> O {
        type Item = O;
    ...
    impl<'a,I: Iterator,F,O,const D: usize> EvalsToRowVec<D> for RowVecMapAndStore<'a,I,F,O,D> where for<'b> F: FnMut(&'b I::Item) -> O {
        type Item = O;
    ...
    impl<I: Iterator,const D: usize> EvalRowVec<I,D> {
        pub fn map_and_store<'a,F,O>(self,f: F,buffer: &'a mut BufferedRowVec<I::Item,D>) -> EvalRowVec<RowVecMapAndStore<'a,I,F,O,D>,D> where for<'b> F: FnMut(&'b I::Item) -> O {
            EvalRowVec(RowVecMapAndStore{
                iter: self.0,
                f,
                buffer
            })
    ...
    //mut map_and_store
    pub struct RowVecMutMapAndStore<'a,I: Iterator,F,O,const D: usize> where for<'b> F: FnMut(&'b mut I::Item) -> O {iter: I, f: F, buffer: &'a mut BufferedRowVec<I::Item,D> }

    impl<'a,I: Iterator,F,O,const D: usize> Iterator for RowVecMutMapAndStore<'a,I,F,O,D> where for<'b> F: FnMut(&'b mut I::Item) -> O {
        type Item = O;
    ...
    impl<'a,I: Iterator,F,O,const D: usize> EvalsToRowVec<D> for RowVecMutMapAndStore<'a,I,F,O,D> where for<'b> F: FnMut(&'b mut I::Item) -> O {
        type Item = O;
    ...
    impl<I: Iterator,const D: usize> EvalRowVec<I,D> {
        pub fn mut_map_and_store<'a,F,O>(self,f: F,buffer: &'a mut BufferedRowVec<I::Item,D>) -> EvalRowVec<RowVecMutMapAndStore<'a,I,F,O,D>,D> where for<'b> F: FnMut(&'b mut I::Item) -> O {
            EvalRowVec(RowVecMutMapAndStore{
                iter: self.0,
                f,
                buffer
            })
    ...
    //neg
    pub struct NegatedRowVec<I: Iterator,const D: usize>(I) where I::Item: Neg;

    impl<I: Iterator,const D: usize> Iterator for NegatedRowVec<I,D> where I::Item: Neg {
        type Item = <I::Item as Neg>::Output;
    ...
    impl<I: Iterator,const D: usize> EvalsToRowVec<D> for NegatedRowVec<I,D> where I::Item: Neg {
        type Item = <I::Item as Neg>::Output;
    ...
    impl<I: IntoIterator,const D: usize> Neg for RowVec<I,D> where I::Item: Neg {
        type Output = EvalRowVec<NegatedRowVec<I::IntoIter,D>,D>;

        #[inline]
        fn neg(self) -> Self::Output {
            EvalRowVec(NegatedRowVec(self.0.into_iter()))
        }
    }

    impl<'a,I: IntoIterator,const D: usize> Neg for &'a RowVec<I,D> where &'a I: IntoIterator, <&'a I as IntoIterator>::Item: Neg{
        type Output = EvalRowVec<NegatedRowVec<<&'a I as IntoIterator>::IntoIter,D>,D>;
    ...
    impl<I: Iterator,const D: usize> Neg for EvalRowVec<I,D> where I::Item: Neg {
        type Output = EvalRowVec<NegatedRowVec<I,D>,D>;

        #[inline]
        fn neg(self) -> Self::Output {
            EvalRowVec(NegatedRowVec(self.0))
        }
    }
}

pub mod col_n_row_vec_iterators {
