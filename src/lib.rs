...
pub mod trait_specialization_utils {
    ...
    impl TyBoolPair for (N, N) {
        type And = N;
        type Or = N;
        type Xor = N;
    }
    impl TyBoolPair for (N, Y) {
        type And = N;
        type Or = Y;
        type Xor = Y;
    }
    impl TyBoolPair for (Y, N) {
        type And = N;
        type Or = Y;
        type Xor = Y;
    }
    impl TyBoolPair for (Y, Y) {
        type And = Y;
        type Or = Y;
        type Xor = N;
    }
    ...
    impl Filter for N {
        type Filtered<T> = ();
        #[inline]
        fn filter<T>(_: T) -> Self::Filtered<T> {}
    }
    impl Filter for Y {
        type Filtered<T> = T;
        #[inline]
    ...
    impl FilterPair for (N, N) {
        ...
        #[inline]
        fn filter<T1, T2>(_: T1, _: T2) -> Self::Filtered<T1, T2> {}
        #[inline]
        unsafe fn defilter<T1, T2>(_: Self::Filtered<T1, T2>) -> (T1, T2) {
            unsafe { (transmute_copy(&()), transmute_copy(&())) }
        }
    }
    impl FilterPair for (N, Y) {
        ...
        #[inline]
        fn filter<T1, T2>(_: T1, x: T2) -> Self::Filtered<T1, T2> {
            x
        }
        #[inline]
        unsafe fn defilter<T1, T2>(filtered: Self::Filtered<T1, T2>) -> (T1, T2) {
            unsafe { (transmute_copy(&()), filtered) }
        }
    }
    impl FilterPair for (Y, N) {
        ...
        #[inline]
        fn filter<T1, T2>(x: T1, _: T2) -> Self::Filtered<T1, T2> {
            x
        }
        #[inline]
        unsafe fn defilter<T1, T2>(filtered: Self::Filtered<T1, T2>) -> (T1, T2) {
            unsafe { (filtered, transmute_copy(&())) }
        }
    }
    ...
    pub trait SelectPair: TyBoolPair {
        ...
        fn select_ref_mut<'a, T1, T2>(
            x1: &'a mut T1,
            x2: &'a mut T2,
        ) -> &'a mut Self::Selected<T1, T2>;
    ...
    impl SelectPair for (N, N) {
        ...
        #[inline]
        fn select<T1, T2>(_: T1, _: T2) -> Self::Selected<T1, T2> {}
        #[inline]
        unsafe fn deselect<T1, T2>(_: Self::Selected<T1, T2>) -> (T1, T2) {
            unsafe { (transmute_copy(&()), transmute_copy(&())) }
        }
        #[inline]
        fn select_ref<'a, T1, T2>(_: &'a T1, _: &'a T2) -> &'a Self::Selected<T1, T2> {
            Box::leak(Box::new(()))
        } //oh no, leaking a (), a type of size 0, whatever will we do...
        #[inline]
        fn select_ref_mut<'a, T1, T2>(
            _: &'a mut T1,
            _: &'a mut T2,
            ...
            Box::leak(Box::new(()))
        } //although this does assume Rust will realize that the & is useless and elides it
    }
    ...
    impl SelectPair for (N, Y) {
        ...
        #[inline]
        fn select<T1, T2>(_: T1, x: T2) -> Self::Selected<T1, T2> {
            x
        }
        #[inline]
        unsafe fn deselect<T1, T2>(filtered: Self::Selected<T1, T2>) -> (T1, T2) {
        ...
        #[inline]
        fn select_ref<'a, T1, T2>(_: &'a T1, x: &'a T2) -> &'a Self::Selected<T1, T2> {
            x
        }
        #[inline]
        fn select_ref_mut<'a, T1, T2>(
    ...
    impl SelectPair for (Y, N) {
        ...
        #[inline]
        fn select<T1, T2>(x: T1, _: T2) -> Self::Selected<T1, T2> {
            x
        }
        #[inline]
        unsafe fn deselect<T1, T2>(filtered: Self::Selected<T1, T2>) -> (T1, T2) {
        ...
        #[inline]
        fn select_ref<'a, T1, T2>(x: &'a T1, _: &'a T2) -> &'a Self::Selected<T1, T2> {
            x
        }
        #[inline]
        fn select_ref_mut<'a, T1, T2>(
...
pub mod util_traits {
    ...
    impl<T: HasOutput + ?Sized> HasOutput for Box<T> {
        ...
        #[inline]
        unsafe fn output(&mut self) -> Self::Output {
            unsafe { (debox(self)).output() }
        }
        #[inline]
        unsafe fn drop_output(&mut self) {
            unsafe { (debox(self)).drop_output() }
        }
    }
